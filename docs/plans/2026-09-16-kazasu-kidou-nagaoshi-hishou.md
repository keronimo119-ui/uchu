...
| 3 | 地方恒星時の計算が正しい | `__U.lst(new Date('2026-09-16T12:00:00Z'), 0)` ≒ 175.45°（±0.1） |
...
- W3C の回転行列（Z-X'-Y''、α=alpha, β=beta, γ=gamma を rad に）:
  ...
  - **画面の上方向** ＝ R × (+sinθ, cosθ, 0)、θ=`screen.orientation.angle`（無ければ0）を rad に。
    （縦持ちθ=0なら (0,1,0)。当初は −sinθ と書いていたが、2026-09-16 にエミュレーターの回転ベクトルと実機で照合したところ180°逆さまだったので +sinθ に修正。
    横持ち θ=90 は端末を反時計回りに倒した状態で、画面の上＝端末の +x）
...
- 地方恒星時: `JD = Date.now()/86400000 + 2440587.5`, `T = JD - 2451545.0`,
  ...
  検算: 2026-09-16T12:00:00Z・経度0 → **175.45°**（±0.1）。
